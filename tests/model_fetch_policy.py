...
Four properties are checked in the source, and the manifest is checked against
the same rules, because a download block naming an arbitrary host or carrying
no digest would walk around all four.
"""
import json
import re
import sys
...
print(f"Model-fetch policy passed: 1 network-capable source held to HTTPS, an exact-match host "
      f"allow-list and a verified digest; {blocks} declared download blocks covering {files} files, "
      f"every one with a SHA-256 and a byte count.")
